...
const Home = () => {
	...
	return (
		<div className="bg-[#3f4a40]">
			...
			<section className="relative sm:min-h-screen bg-stone-900">
				...
				<div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 min-h-[100svh] sm:min-h-screen flex flex-col justify-end md:justify-center items-center pt-15">
					<div className="text-center mb-auto mt-24 sm:mt-36 md:mt-52 px-3">
						<h1 className="text-[35px] md:text-5xl lg:text-7xl text-white italic tracking-wide whitespace-nowrap pb-10 " style={{ fontFamily: "'Great Vibes', cursive" }}>
			...
			{/* Your Services - Mobile Swipeable Section */}
			<section id="discover" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 relative overflow-hidden">
				{/* Header Section */}
				<div className="mb-10 grid grid-cols-1 lg:grid-cols-[1fr_1fr] items-start gap-10">
					...
					<div className="flex flex-col items-start lg:items-end text-left lg:text-right pt-1">
				...
				{servicesLoading && (
					<div className="mb-1 rounded-xl border border-stone-200 bg-white px-4 py-3 text-sm text-[#efece6]">
						Loading services...
					</div>
				)}
				{servicesError && (
					<div className="mb-1 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-[#efece6]">
						{servicesError}
					</div>
				)}
					...
					<>
						...
						<div className="sm:hidden relative">
							<div className="overflow-x-auto scrollbar-hide snap-x snap-mandatory scroll-smooth pb-4 -mx-2 px-1">
								<div className="flex gap-4 w-max">
									{servicesState.slice(0, 4).map((service, index) => (
										<div
											key={index}
											className="snap-start w-[72vw] max-w-[260px] flex-shrink-0 flex flex-col h-full bg-[#3f4a40]/80 rounded-2xl p-3 backdrop-blur-sm"
										>
											{/* Smaller Arched Container for Mobile */}
											<div className="relative mb-3">
												<div className="w-full aspect-[3/4] rounded-[120px] rounded-b-2xl overflow-hidden shadow-md">
													{resolveServiceVideo(service) ? (
														<video
															src={resolveServiceVideo(service)}
															className="w-full h-full object-cover"
											...
											<div className="flex-1 flex flex-col items-center text-center">
												<h3 className="text-base font-serif text-[#efece6] mb-0.5 line-clamp-1">{service.name}</h3>
												<p className="text-[10px] text-[#c9c3b6] font-medium uppercase tracking-wider mb-2">{service.category || 'Premium Service'}</p>

												{/* Smaller Book Now Button */}
												<Link to="/services" className="mt-auto w-full">
													<Button className="rounded-lg w-full px-3 h-8 text-[10px] font-semibold uppercase tracking-wide bg-stone-900 text-white hover:bg-stone-800 transition-colors">
														Book Now
													</Button>
												</Link>
											</div>
							...
							<div className="flex justify-center gap-1 mt-4">
								{servicesState.slice(0, 4).map((_, idx) => (
									<div
										key={idx}
					...
					<div className="mt-6 flex items-center justify-center gap-3">
						{filteredAccommodations.map((_, index) => (
							<button
								...
								className={`h-1 rounded-full transition-all ${index === accommodationIndex ? 'w-6 bg-amber-400' : 'w-4 bg-stone-300'
									}`}
